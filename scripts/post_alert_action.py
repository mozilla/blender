...
Writes a JSON summary to .blender-alert-summary.json for upload as a
workflow artifact (visible only to users with Actions access).
...
SUMMARY_FILE = ".blender-alert-summary.json"
REQUIRED_KEYS = {
    "affected",
    "confidence",
    "reason",
...
def write_summary(
    ...
    """Write a JSON summary file for upload as a workflow artifact."""
    summary = {
...
def main() -> None:
    ...
    write_summary(SUMMARY_FILE, alert_number, package_name, severity, action, reason)
